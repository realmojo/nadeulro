...
/**
 * '하루 코스' 조합에 넣을 만한 곳인지 — 어른 나들이 기준.
 * 어린이 전용 수영장은 시설 정보로는 유효하지만 어른 나들이 조합으로
 * 제안하면 엉뚱해지므로 코스 계산에서만 뺀다(상세 페이지는 그대로 유지).
 */
export function isCoursePairable(p: Place): boolean {
  if (p.category === "swim" && KIDS_ONLY.test(p.name)) return false;
  return true;
}
